...
void controlMotor() {
  // 控制馬達
  lf_pwm = controller.vel_command(0, desired_vel[0]);
  lb_pwm = controller.vel_command(1, desired_vel[1]);
  rf_pwm = controller.vel_command(2, desired_vel[2]);
  rb_pwm = controller.vel_command(3, desired_vel[3]);

  Serial2.println(
    "<" + direction_[0] + lf_pwm + direction_[1] + lb_pwm + direction_[2] + rf_pwm + direction_[3] + rb_pwm);
}
...
void serialEvent()
{
  ...
  switch (str) {
    // 傳送機器人資訊給nano
    case 'a':
      data = "a" + String(direction_[0]) + String(motor_vel[0]) + " " + String(direction_[1]) + String(motor_vel[1]) + " " + String(direction_[2]) + String(motor_vel[2]) + " " + String(direction_[3]) + String(motor_vel[3]) + "|" + \
             String(motor_degree[0]) + " " + String(motor_degree[1]) + " " + String(motor_degree[2]) + " " + String(motor_degree[3]) + "|" + \
